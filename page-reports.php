...
document.querySelectorAll('form').forEach(function(form){
    ...
    cc.addEventListener('change',function(){sci=this.value;ct.innerHTML='<option value=\"\">区/县</option>';ct.disabled=true;cl();if(!this.value||!addrData[sc][sp]||!addrData[sc][sp][1][this.value])return;var cd=addrData[sc][sp][1][this.value];sl(cd[0],cd[1]);cd.slice(2).forEach(function(d){ct.add(new Option(typeof d==='string'?d:d[0],typeof d==='string'?d:d[0]));});ct.disabled=false;});
    ct.addEventListener('change',function(){if(!this.value||!addrData[sc]||!addrData[sc][sp]||!addrData[sc][sp][1][sci])return;var cd=addrData[sc][sp][1][sci];var found=null;cd.slice(2).forEach(function(d){if((typeof d==='string'?d:d[0])===ct.value)found=d;});if(found&&typeof found!=='string')sl(found[1],found[2]);else sl(cd[0],cd[1]);});
});
</script>
<?php get_footer(); ?>
